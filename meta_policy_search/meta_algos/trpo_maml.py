...
class TRPOMAML(MAMLAlgo):
    ...
    def _adapt_objective_sym(self, action_sym, adv_sym, dist_info_old_sym, dist_info_new_sym):
        if self.inner_type == 'likelihood_ratio':
            ...
            with tf.variable_scope("surrogate_loss"):
                surr_obj_adapt = - \
                    tf.reduce_mean(likelihood_ratio_adapt * adv_sym)
            ...
            with tf.variable_scope("surrogate_loss"):
                surr_obj_adapt = - \
                    tf.reduce_mean(log_likelihood_adapt * adv_sym)
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name):
            ...
            self.adapted_policies_params, self.adapt_input_ph_dict, self.loss_list, self.adapt_input_ph_dict_first, self.grads_list = self._build_inner_adaption()
        ...
        with tf.variable_scope(self.name):
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                ...
                for i in range(self.meta_batch_size):
                    ...
                    adapted_params_var, _ = self._adapt_sym(
            ...
            for i in range(self.meta_batch_size):
                ...
                if self.exploration:
                    ...
                    adj_avg_rewards = tf.placeholder(dtype=tf.float32, shape=[
                                                     None], name='adj_avg_rewards' + '_' + str(self.num_inner_grad_steps) + '_' + str(i))
                    self.meta_op_phs_dict['step%i_task%i_%s' % (
                        self.num_inner_grad_steps, i, 'adj_avg_rewards')] = adj_avg_rewards

                    log_likelihood_inital = self.policy.distribution.log_likelihood_sym(initial_action_phs[i],
                                                                                        initial_distribution_info_vars[i])
                    surr_obj += - \
                        tf.reduce_mean(adj_avg_rewards) * \
                        tf.reduce_mean(log_likelihood_inital)
