...
class ProMP(MAMLAlgo):
    ...
    def __init__(
        ...
        self.optimizer = MAMLPPOOptimizer(
            learning_rate=learning_rate, max_epochs=num_ppo_steps, num_minibatches=num_minibatches)
        self.clip_eps = clip_eps
        self.target_inner_step = target_inner_step
        self.adaptive_inner_kl_penalty = adaptive_inner_kl_penalty
        self.inner_kl_coeff = init_inner_kl_penalty * \
            np.ones(self.num_inner_grad_steps)
        self.anneal_coeff = 1
        self.anneal_factor = anneal_factor
        self._optimization_keys = ['observations',
                                   'actions', 'advantages', 'agent_infos']
        self.name = name
        self.kl_coeff = [init_inner_kl_penalty] * \
            self.meta_batch_size * self.num_inner_grad_steps
    ...
    def build_graph(self):
        ...
        with tf.variable_scope(self.name):
            ...
            for step_id in range(1, self.num_inner_grad_steps+1):
                ...
                for i in range(self.meta_batch_size):
                    surr_loss = self._adapt_objective_sym(
                        action_phs[i], adv_phs[i], dist_info_old_phs[i], distribution_info_vars[i])
                    kl_loss = tf.reduce_mean(self.policy.distribution.kl_sym(
                        dist_info_old_phs[i], distribution_info_vars[i]))
            ...
            mean_inner_kl_per_step = tf.stack(
                [tf.reduce_mean(tf.stack(inner_kls)) for inner_kls in all_inner_kls])

            """ Outer objective """
            surr_objs, outer_kls = [], []

            # Create placeholders
            inner_kl_coeff = tf.placeholder(
                tf.float32, shape=[self.num_inner_grad_steps], name='inner_kl_coeff')
            ...
            for i in range(self.meta_batch_size):
                ...
                outer_kl = tf.reduce_mean(self.policy.distribution.kl_sym(
                    dist_info_old_phs[i], distribution_info_vars[i]))
            ...
            inner_kl_penalty = tf.reduce_mean(
                inner_kl_coeff * mean_inner_kl_per_step)

            """ Mean over meta tasks """
            meta_objective = tf.reduce_mean(
                tf.stack(surr_objs, 0)) + inner_kl_penalty
    ...
    def optimize_policy(self, all_samples_data, log=True):
        ...
        if self.adaptive_inner_kl_penalty:
            ...
            self.inner_kl_coeff = self.adapt_kl_coeff(
                self.inner_kl_coeff, inner_kls, self.target_inner_step)
